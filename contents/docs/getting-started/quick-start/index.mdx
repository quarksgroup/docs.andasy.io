...
**Andasy** is a modern cloud Platform-as-a-Service (PaaS) that helps developers deploy and manage applications without touching infrastructure. 

## What is Platform-as-a-Service (PaaS)?

**Platform-as-a-Service (PaaS)** is a cloud computing model where a provider manages the underlying infrastructure (servers, networking, operating systems) and provides a platform for you to deploy and run applications. With Andasy:

- **No infrastructure management** - You don't need to configure servers, networking, or operating systems
- **Focus on code** - Write your application code and deploy it; Andasy handles the rest
- **Automatic scaling** - Applications can scale to handle varying loads
- **Built-in services** - Databases, logging, metrics, and monitoring are included

Whether you're running a simple API or a complex web app, Andasy handles builds, deployments, networking, scaling, and observability so you don't have to.
...
If you haven’t already, [sign up for a free Andasy account](https://andasy.io/auth/signup).
...
By default, your app is deployed into a specific region. You can control this explicitly by setting a region, for example:

```bash
ANDASY_REGION=kgl andasy deploy
```

See the [Regions](/docs/reference/regions) guide for more details on how to list available regions and choose the right one for your app.
