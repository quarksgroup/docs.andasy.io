...
- **Application block** - Contains the detailed deployment configuration:
  ...
  - **Region settings** - Which regions your application is deployed to
...
| `primary_region` | String | The primary deployment region code for your application (for example `\"kgl\"`). This is used when no explicit `regions` list is provided. | Yes |
| `regions` | List of strings | Optional list of region codes for multi-region deployments. If omitted, Andasy uses `primary_region` as the single deployment region. | No |
